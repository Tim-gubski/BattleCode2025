package v52_SleepyBAD.Units;

import v52_SleepyBAD.Unit;
...
public class Soldier extends Unit {
    ...
    final int SRP_TOWER_THRESHOLD = Math.max(4,(int)Math.round(6 - Math.min(rc.getMapWidth(),rc.getMapHeight())/10.0));
...
//        tryMove(DPPathFinder.computeMove(mapData.mapInfos, rc.getLocation(), new MapLocation(0,0), 2));
    ...
    private UnitState determineState() throws GameActionException{
        ...
        if (closestCompletableRuin != null && shouldBuildTower){//UnitType.LEVEL_ONE_PAINT_TOWER.paintCost - 100) {
            return UnitState.BUILD;
        }
...
//        else{
//            MapLocation[] srpSpots = srpGridLocs(rc.getLocation());
//            for(MapLocation loc : srpSpots){
            ...
            if (!checkAndPaintTile(rc.getLocation()) && rc.isActionReady()) {
                ...
                if(rc.isActionReady()) {
                    ...
                    if(closestCompletableRuin != null){
                        ...
                        MapLocation[] spiral = ruinFillOrder(closestCompletableRuin);
                        for (int i = 0; i < spiral.length; i++) {
                            if (Clock.getBytecodesLeft() < 1000 || checkAndPaintTile(spiral[i])) {
...
//            }
    ...
    final int BLACKLIST_DURATION = 50;
    private UnitState exploreState() throws GameActionException {
        ...
        if(rc.isMovementReady()){
            ...
            if(shouldBuildTower){
                // find some empty ruins
                mapData.ruins.updateIterable();
                for(MapLocation ruin : mapData.ruins.locs){
                    boolean alreadyHasTower = false;
                    for(RobotInfo tower : mapData.getFriendlyTowers()){
                        if(tower.location.equals(ruin)){
                            alreadyHasTower = true;
                            break;
                        }
                    }
                    if(alreadyHasTower){
                    ...
                    if(rc.getRoundNum() - roundVisited > BLACKLIST_DURATION){
...
//            }
    ...
    private UnitState buildState() throws GameActionException {
        ...
        if(markDir != null && rc.canSenseLocation(closestCompletableRuin)){
            UnitType towerType = determineTowerPattern(closestCompletableRuin);
        ...
        if(rc.isMovementReady()) {
            if(distTo(closestCompletableRuin) <= 2){
                for(MapLocation checkLoc : ruinFillOrder(closestCompletableRuin)) {
                    MapInfo checkLocInfo = mapData.getMapInfo(checkLoc);
                    if(checkLocInfo != null && checkLocInfo.getPaint() == PaintType.EMPTY) {
                        MapLocation bestLoc = rc.getLocation();
                        int bestScore = tileScore(rc.getLocation(), true)-1;
                        for (Direction dir : directions) {
                            MapLocation loc = closestCompletableRuin.add(dir);
//                    debugString.append("\nDir: " + dir + " Score: " + tileScore(loc, true));
                            int score = tileScore(loc, true);
                            if (score > bestScore
                                    && rc.canMove(dirTo(loc))
                                    && distTo(loc) <= 2
                                    && loc.distanceSquaredTo(checkLoc) <= UnitType.SOLDIER.actionRadiusSquared) {
                                bestScore = score;
                                bestLoc = loc;
                            }
                        }
                        debugString.append("\nBest Score: " + bestScore + " Best Loc: " + bestLoc);
                        if (bestLoc != null) {
                            if(bestLoc.equals(rc.getLocation()) && distTo(checkLoc) <= UnitType.SOLDIER.actionRadiusSquared) {
                                standStill = true;
                            }else if(!bestLoc.equals(rc.getLocation())){
...
//            }
        ...
        if(rc.isMovementReady() && !standStill){
            if(distTo(closestCompletableRuin) > 8) {
