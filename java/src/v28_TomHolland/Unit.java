    ...
    protected UnitState refillingState() throws GameActionException {
        ...
        if (targetTower == null) {
            if(spawnTower != null && !checkedHome){
                safeFuzzyMove(spawnTower, enemies);
        ...
        if (rc.canSenseRobotAtLocation(targetTower)){
            ...
            if (tower.getPaintAmount() < 30 && paintTowers.length > 1){
                ...
                if (paintTowers[targetTowerIndex + 1] != null && distTo(paintTowers[targetTowerIndex + 1]) < Math.pow((rc.getType().paintCapacity * 0.75 - rc.getPaint()) / rate, 2)) {
...
//        }
        ...
        if (rc.getLocation().isWithinDistanceSquared(targetTower, 18)) {
            ...
            if(tower.getPaintAmount() > REFILL_THRESHOLD || tower.type == UnitType.LEVEL_ONE_MONEY_TOWER){
...
//            }
    ...
    public void bugNav(MapLocation loc) throws GameActionException{
        ...
        if(bugStackIndex == 0){
            ...
            if(rc.canMove(dirToTarget) && score > bestScore){
                bestDir = dirToTarget;
                bestScore = score;
            }
            if(rc.canMove(dirToTarget.rotateLeft()) && (score = tileScore(rc.getLocation().add(dirToTarget.rotateLeft()), false)) > bestScore){
                bestDir = dirToTarget.rotateLeft();
                bestScore = score;
            }
            if(rc.canMove(dirToTarget.rotateRight()) && (score = tileScore(rc.getLocation().add(dirToTarget.rotateRight()), false)) > bestScore){
