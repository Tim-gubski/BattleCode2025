...
public abstract class Tower extends Robot {
    ...
    public void turn() throws GameActionException {
        if (rc.getRoundNum() < 3) {
            ...
            Direction dir = dirTo(new MapLocation(rc.getMapWidth() - loc.x - 1, rc.getMapHeight() - loc.y - 1));
            if (!trySummon(UnitType.SOLDIER, loc.add(dir).add(dir))) {
                trySummon(UnitType.SOLDIER, loc.add(dir));
            }
        }
        ...
        if (returnLoc != null) {
            debugString.append("@frontline=").append(returnLoc);
...
//        }
        ...
        if(rc.getRoundNum()<40){
            ...
            double soldierChance    = 0.4 * Math.min(1, 50 / (rc.getRoundNum() * 1.2));
            double mopperChance     = soldierChance + 0.3;
            if(rand < soldierChance) {
                return trySummon(UnitType.SOLDIER);
            } else if (rand < mopperChance) {
                return trySummon(UnitType.MOPPER);
            } else {
                return trySummon(UnitType.SPLASHER);
            }
