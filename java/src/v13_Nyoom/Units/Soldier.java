...
public class Soldier extends Unit {
    ...
    public void turn() throws GameActionException {
        ...
        if (rc.getRoundNum() < 4 && turnsAlive == 0) {
            rusher = true;
            if (rc.getMapWidth() == 20) {
                if (rc.getTeam() == Team.A) {
                    rushLoc = new MapLocation(9, 17);
                } else {
                    rushLoc = new MapLocation(10, 2);
                }
            } else if (rc.getMapWidth() == 35) {
                if (rc.getTeam() == Team.A) {
                    rushLoc = new MapLocation(27, 7);
                } else {
                    rushLoc = new MapLocation(7, 7);
                }
            } else if (rc.getMapWidth() == 50) {
                if (rc.getTeam() == Team.A) {
                    rushLoc = new MapLocation(41, 17);
                } else {
                    rushLoc = new MapLocation(8, 17);
                }
            } else if (rc.getMapWidth() == 59) {
                if (rc.getTeam() == Team.A) {
                    rushLoc = new MapLocation(31, 54);
                } else {
                    rushLoc = new MapLocation(31, 4);
                }
            } else {
                throw new RuntimeException("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAH");
            }
        }
...
//        System.out.println("Bytecodes used for nearestNeighbor: " + (bytecode - Clock.getBytecodesLeft()));
    ...
    private UnitState determineState() throws GameActionException{
        ...
        if (closestCompletableRuin != null && rc.getChips() >= 700){//UnitType.LEVEL_ONE_PAINT_TOWER.paintCost - 100) {
...
//        System.out.println("Bytecodes used for finding srp: " + (startByte - Clock.getBytecodesLeft()));
    ...
    private void stateInvariantActions() throws GameActionException{
        ...
        if(state != UnitState.REFILLING && !rusher) {
...
//            RobotInfo tower = inEnemyTowerRange(enemies);
    ...
    private UnitState rushState() throws GameActionException {
        debugString.append("Rushing to ").append(rushLoc);
        trySetIndicatorDot(rushLoc, 255, 0, 0);
        fuzzyMove(rushLoc);
        RobotInfo currentTargetRobot = rc.canSenseRobotAtLocation(rushLoc) ? rc.senseRobotAtLocation(rushLoc) : null;
        int nearest = distTo(rushLoc);
        for (RobotInfo enemy : rc.senseNearbyRobots(-1, rc.getTeam().opponent())) {
            trySetIndicatorDot(enemy.location, 0, 255, 0);
            int dist = distTo(enemy.location);
            if (isMoneyTower(enemy.getType()) && (dist < nearest || currentTargetRobot == null || currentTargetRobot.getType().isTowerType())) {
                nearest = dist;
                rushLoc = enemy.location;
            }
        }
        if (rc.canSenseLocation(rushLoc) && rc.senseRobotAtLocation(rushLoc) == null || !isMoneyTower(rc.senseRobotAtLocation(rushLoc).type)) {
            rusher = false;
        } else {
            tryAttack(rushLoc);
        }
