package v10_YapYapYap;

import v10_YapYapYap.Helpers.Explore;
import v10_YapYapYap.Helpers.MapData;
import v10_YapYapYap.Util.Comms;
...
public abstract class Unit extends Robot {
    ...
    protected void senseNearby() throws GameActionException {
        ...
        if (enemies != null) returningFromFight = true;
...
//            }
        ...
        if (returningFromFight && rc.canSendMessage(foundTower)) communication.constructMessage(Comms.Codes.FRONTLINE);
        
        // if in range of tower, refill
        if (rc.getLocation().isWithinDistanceSquared(foundTower, 2)) {
            refillSelf();
            trySetIndicatorDot(foundTower, 0, 255, 0);
            // done filling
        ...
        if (rc.getLocation().distanceSquaredTo(foundTower) > 2) {
            trySetIndicatorDot(foundTower, 255, 255, 255);
    ...
    public boolean tryExploreOnPaint() throws GameActionException {
        ...
        for(Direction dir : fuzzyDirs(paintExploreDirection)){
            ...
            if(rc.canSenseLocation(loc)){
                MapInfo info = rc.senseMapInfo(loc);
                trySetIndicatorDot(loc, 0, 0, 255);
    ...
    public boolean isSafe(MapLocation loc, RobotInfo[] enemies) throws GameActionException {
        ...
        for (RobotInfo enemy : enemies) {
            ...
            if (loc.isWithinDistanceSquared(enemy.getLocation(), enemy.getType().actionRadiusSquared)) {
                trySetIndicatorDot(enemy.getLocation(), 255, 0, 0);
                return false;
            }
        }
        ...
        if ((isEnemyPaint(info.getPaint()) && rc.getPaint() < rc.getType().paintCapacity * 0.4) ||
            ...
            trySetIndicatorDot(info.getMapLocation(), 255, 0, 0);
            return false;
        }
    ...
    public void bugNav(MapLocation loc) throws GameActionException{
        trySetIndicatorLine(rc.getLocation(),loc,0,255,0);
        if(!rc.isMovementReady()){
            return;
        }
        //reset
    ...
    public boolean checkAndPaintTile(MapInfo info) throws GameActionException{
        ...
        if ((info.getPaint() == PaintType.EMPTY
            ...
            if(closestAnyRuin != null){
                trySetIndicatorDot(closestAnyRuin, 0, 255, 255);
            }
            trySetIndicatorDot(info.getMapLocation(), 255, 0, 255);
            return true;
        }
        return false;
    }
