...
public abstract class Tower extends Robot {
    ...
    public Tower(RobotController robot) throws GameActionException {
        ...
        diagonalLoc = new MapLocation(width-rc.getLocation().x, height-rc.getLocation().y);
        enemyDirection = rc.getLocation().directionTo(diagonalLoc);
...
//                rc.setIndicatorLine(ally.location, friendlyTowers[i].location, 0, 150, 150);
        ...
        if(soldiers >= 1 && moppers < soldiers && (rc.getRoundNum() - emergencyMopperTurn > MOPPER_COOLDOWN || mopperCount < soldiers) && lastMoney != currentMoney){// && rc.getRoundNum() < 100){
                    ...
                    if (allies.length < 3 && (((spawnTurn - startSpawnTurn < 2) && (firstTower || rc.getNumberTowers() > 5)) || rc.getChips() > 2100)) {
                        if (spawnTurn % 3 < 2) {
                            ...
                            if (startSpawnTurn - spawnTurn == 0) {
                                ...
                                MapLocation edgeLoc = extendLocToEdge(rc.getLocation(), enemyDirection.opposite());
                                rc.setIndicatorLine(rc.getLocation(), edgeLoc, 255, 0, 0);
                                rc.setIndicatorLine(rc.getLocation(), diagonalLoc, 0, 0, 255);
                                if (distTo(diagonalLoc) <= distTo(edgeLoc)) {
                                    dir = dir.opposite();
                                }
                                if (trySummonInDirection(UnitType.SOLDIER, dir)) {
...
//            }else if(rc.getChips()>1000){
        ...
        }else if(rc.getType().getBaseType() != UnitType.LEVEL_ONE_PAINT_TOWER && (rc.getNumberTowers()>=4 || rc.getChips() > 2000) && rc.getChips() > 1100){
