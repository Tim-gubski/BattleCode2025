...
public abstract class Unit extends Robot {
    ...
    public RobotInfo[] enemyTowers = null;
...
//        int bytecode = Clock.getBytecodeNum();
        if(rc.getRoundNum() - spawnTurn > 1 && rc.getType() == UnitType.SOLDIER) {
...
//        bytecode = Clock.getBytecodeNum() - bytecode;
        ...
        enemyTowers = getEnemyTowers(enemies);
...
//        closestCompletableRuin = getClosest(completableRuins);
        ...
        mapData.updateLandmarks(allRuins, allies, enemies, rc.getLocation()); // usually low but i saw it spike to 7k???
...
//    }
    ...
    public MapLocation cuckLocation = null;
    public MapLocation[] senseNearbyCompletableTowerlessRuins() throws GameActionException{
        MapLocation[] nearbyRuins = rc.senseNearbyRuins(-1);
        int mostComplete = -1;
        MapLocation mostCompleteLoc = null;
        int toRemove = 0;
        for (int i = 0; i < nearbyRuins.length; i++){
            if(rc.senseRobotAtLocation(nearbyRuins[i]) != null){
                mapData.visitedRuins.put(nearbyRuins[i], rc.getRoundNum());
                nearbyRuins[i] = null;
                toRemove++;
                continue;
            }
            ...
            RobotInfo[] workingFriends = rc.senseNearbyRobots(nearbyRuins[i], 32, rc.getTeam());
...
//                if (nearbyRuins[i] == null) break;
                for(int y = -2; y <= 2; y++){
                    ...
                    if(rc.onTheMap(senseLoc) && tileInfo != null){
                        sensable++;
                        if(!enemyPaint && tileInfo.getPaint().isEnemy() && !haveMoppers){
                            if(rc.getType()!=UnitType.MOPPER) {
                                needMoppers = tileInfo.getMapLocation();
                                rc.setIndicatorLine(rc.getLocation(), needMoppers, 0, 0, 255);
                            }
                            enemyPaint = true;
//                            nearbyRuins[i] = null;
//                            toRemove++;
//                            break;
                        ...
                        int targetColorInt = mapData.tileColors[senseLoc.x][senseLoc.y] - 1;
                        if(tileInfo.getPaint() == (targetColorInt == 1 ? PaintType.ALLY_SECONDARY : PaintType.ALLY_PRIMARY)){
                            correctPaint++;
                        }
...
//            debugString.append("Working Friends: " + soldiers + " Correct Paint: " + correctPaint + " Sensable: " + sensable + "\n");
            ...
            if(correctPaint > mostComplete && nearbyRuins[i] != null){
                mostComplete = correctPaint;
                mostCompleteLoc = nearbyRuins[i];
            }
        }
        if(mostCompleteLoc != null){
            return new MapLocation[]{mostCompleteLoc};
        }
...
//        debugString.append("FAILED REFILL 5");
    ...
    protected UnitState refillingState() throws GameActionException {
        ...
        for(RobotInfo ally : allies){
            ...
            }else if(ally.getType().getBaseType() == UnitType.LEVEL_ONE_PAINT_TOWER){
                if(ally.getPaintAmount() > 50 && rc.getLocation().distanceSquaredTo(ally.location) < minDist){
                    minDist = rc.getLocation().distanceSquaredTo(ally.location);
                    closestTower = ally.location;
                }
            }
        ...
        if (rc.getLocation().isWithinDistanceSquared(targetTower, 18)) {
            ...
            // wait until you can refill
                ...
                if(rc.getType() == UnitType.SOLDIER && rc.getPaint()>6){
                    ...
                    for(RobotInfo ally : allies){
                        if((ally.getType() == UnitType.MOPPER || ally.getType() == UnitType.SPLASHER) && mapData.getMapInfo(ally.location).getPaint() == PaintType.EMPTY){
...
//            rc.setIndicatorDot(rc.getLocation().add(dir), 0, 100, 100);
        ...
        if(bestDir != Direction.CENTER && (!avoidTowers || bestDirScore > -20) && bestDirScore > -1000){
            rc.move(bestDir);
            return true;
        }
...
//            rc.setIndicatorDot(info.getMapLocation(), 255, 0, 0);
    ...
    public int tileScore(MapLocation loc, boolean ignoreEmpty, boolean checkAllyBehind) throws GameActionException{
        ...
        int score = 0;
        MapInfo info = mapData.getMapInfo(loc);
        if(tooCloseToEnemyTower(loc)){
            score -= 20;
        }
        if(mapData.enemyDefenseTowers.size() != 0){
            for(RobotInfo enemyTower : mapData.enemyDefenseTowers.getArray()){
                if(enemyTower.location.distanceSquaredTo(loc) <= enemyTower.type.actionRadiusSquared){
                    score -= 10000;
                    break;
                }
            }
...
//        debugString.append("last: " + lastLocation + " current: " + currentLocation);
        ...
        if(reachableFrom(rc.getLocation(), loc)){
            bugStack = new Direction[MAX_STACK_SIZE];
            bugStackIndex = 0;
        }
        ...
        if(bugStackIndex == 0){
            ...
            Direction bestDir = null;
            int bestScore = -9999;
            boolean ignoreEmpty = rc.getType() == UnitType.SOLDIER && state != UnitState.REFILLING;
            int score1 = tileScore(rc.getLocation().add(dirToTarget), ignoreEmpty);
            int score2 = tileScore(rc.getLocation().add(dirToTarget.rotateLeft()), ignoreEmpty);
            int score3 = tileScore(rc.getLocation().add(dirToTarget.rotateRight()), ignoreEmpty);

            // jiggle if cardinal
            if(dirToTarget.ordinal() % 2 == 0 && distTo(loc) > 50){
                if(rc.getRoundNum()%4 < 2) score2++;
                else score3++;
            }
            if(rc.canMove(dirToTarget) && score1 > bestScore && !rc.getLocation().add(dirToTarget).equals(lastLocation)){
                bestDir = dirToTarget;
                bestScore = score1;
            }
            if(rc.canMove(dirToTarget.rotateLeft()) && score2 > bestScore && !rc.getLocation().add(dirToTarget.rotateLeft()).equals(lastLocation)){
                bestDir = dirToTarget.rotateLeft();
                bestScore = score2;
            }
            if(rc.canMove(dirToTarget.rotateRight()) && score3 > bestScore && !rc.getLocation().add(dirToTarget.rotateRight()).equals(lastLocation)){
...
//                    debugString.append("Wall Following ");
    ...
    public boolean tooCloseToEnemyTower(MapLocation loc) throws GameActionException{
        for(RobotInfo enemyTower : enemyTowers){
            if(enemyTower.location.distanceSquaredTo(loc) <= enemyTower.type.actionRadiusSquared){
                return true;
            }
        }
...
//        return (closestEnemyTower != null && loc.distanceSquaredTo(closestEnemyTower.location) <= closestEnemyTower.type.actionRadiusSquared);
    ...
    public MapLocation[] ruinFillOrder(MapLocation loc){
        int x = loc.x;
        int y = loc.y;
    ...
    public MapLocation[] r3spiral(int x, int y){
        return new MapLocation[]{
    ...
    public MapLocation[] r2spiral(int x, int y){
        return new MapLocation[]{new MapLocation(0, 0).translate(x, y),
                new MapLocation(-1, 1).translate(x, y),
                new MapLocation(0, 1).translate(x, y),
                new MapLocation(1, 1).translate(x, y),
                new MapLocation(1, 0).translate(x, y),
    ...
    public MapLocation[] mapLocationSpiral(MapLocation loc, int radius){
        if(radius == 3){
            return r3spiral(loc.x, loc.y);
        }else if(radius == 2){
            return r2spiral(loc.x, loc.y);
        }
        return r3spiral(loc.x, loc.y);
    }

    public RobotInfo[] getEnemyTowers(RobotInfo[] enemies) {
        int num = 0;
        for(RobotInfo enemy : enemies){
            if(enemy.getType().isTowerType()){
                num++;
        ...
        RobotInfo[] towers = new RobotInfo[num];
        for(RobotInfo enemy : enemies){
            if(enemy.getType().isTowerType()){
                towers[--num] = enemy;
            }
    ...
    public RobotInfo inEnemyTowerRange(RobotInfo[] enemies) throws GameActionException {
        ...
        for (RobotInfo enemy : enemies) {
            if (enemy.getType().isTowerType()) {
                if (rc.getLocation().distanceSquaredTo(enemy.getLocation()) < closestDist) {
                    closestDist = rc.getLocation().distanceSquaredTo(enemy.getLocation());
                    closest = enemy;
                }
            }
        }
...
//        debugString.append("Enemy Paint: " + enemyPaint);
        if(enemyPaint < 2) return null;
