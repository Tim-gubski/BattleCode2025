...
//                else if(enemy.type == UnitType.MOPPER && !mapData.getMapInfo(enemy.location).getPaint().isAlly()){
//                    int x = enemy.location.x - rc.getLocation().x + 5;
//                    int y = enemy.location.y - rc.getLocation().y + 5;
//                    maxAdjacent[x][y] += 5;
//                    maxAdjacent[x + 1][y] += 5;
...
//        System.out.println("Bytecode used for splasher thing: " + (bytecode - Clock.getBytecodesLeft()));
            ...
            for (MapLocation loc : rc.getAllLocationsWithinRadiusSquared(rc.getLocation(), 10)) {
                ...
                if (maxAdjacent[x][y] > max) {// && reachableFrom(rc.getLocation(), loc)){
                    max = maxAdjacent[x][y];
                    bestTile = loc;
                }
            }
            ...
            if (bestTile != null && (max >= 4 || max == maxPossible || (!rc.canSenseRobotAtLocation(closestAnyRuin) && bestTile.distanceSquaredTo(closestAnyRuin) <= 8))) {
                if (distTo(bestTile) > 4) {
                    if (!tryMoveIntoRange(bestTile, 4, false)) {
                        safeFuzzyMove(bestTile, enemies);
                    }
                }
                if (rc.canAttack(bestTile)) {
                    rc.attack(bestTile);
                }
            }
        }
        ...
        if (enemyPaintLoc != null && reachableFrom(rc.getLocation(), enemyPaintLoc) && dirTo(enemyPaintLoc) != Direction.CENTER) {
