package v38_TheRealestMerlin.Units;

import v38_TheRealestMerlin.Unit;
...
//        tryMove(DPPathFinder.computeMove(mapData.mapInfos, rc.getLocation(), new MapLocation(0,0), 2));
    ...
    private UnitState determineState() throws GameActionException{
        ...
        int numSoldiers = 0;
        if(closestEnemyTower != null && reachableFrom(rc.getLocation(), closestEnemyTower.location)){
...
//                    nextWayPoint();
//                }
                if(distTo(exploreLocation) <= 8){// || previousState != UnitState.EXPLORE){
                    for(int tries = 6; --tries >= 0;){
                        exploreDirection = randomDirection();
                        exploreLocation = extendInDir(rc.getLocation(), exploreDirection, width/3);
