...
public abstract class Tower extends Robot {
    ...
    public void turn() throws GameActionException {
        ...
        if(rc.getChips()>3000 && rc.canUpgradeTower(rc.getLocation()) && friendlies.length > 3){
        ...
        if(enemyPaint){
            trySummon(UnitType.SPLASHER);
        }else {
            if (friendlies.length < 5 && (spawnTurn - startSpawnTurn < 10 || rc.getChips() > 1000)) {
                if (spawnTurn % 5 < 2) {
    ...
    public boolean trySummon(UnitType type) throws GameActionException{
        for(MapLocation loc : rc.getAllLocationsWithinRadiusSquared(rc.getLocation(), GameConstants.BUILD_ROBOT_RADIUS_SQUARED)){
            if(trySummon(type, loc)){
                debugString.append("Summoned ").append(type.toString());
                return true;
            }
        }
        return false;
