package v41_SlimJimBAD.Units;

import v41_SlimJimBAD.Unit;
...
public class Soldier extends Unit {
    ...
    final int RUSHDOWN_DISTANCE_SQUARED = 225;

    MapLocation[] enemySpawnLocs = null;
...
//        exploreLocation = extendInDir(rc.getLocation(), exploreDirection, Math.max(MIN_EXPLORE_DIST,width/3));
        ...
        if(spawnTower != null) {
            enemySpawnLocs = new MapLocation[]{new MapLocation(spawnTower.x, height - 1 - spawnTower.y),
                    new MapLocation(width - 1 - spawnTower.x, spawnTower.y),
                    new MapLocation(width - 1 - spawnTower.x, height - 1 - spawnTower.y),
            };
        }
    }
    ...
    public void turn() throws GameActionException {
        ...
        shouldFillAround = state != UnitState.REFILLING && state != UnitState.COMBAT &&
                (mapData.friendlyTowers.paintTowers >= 2 || (state == UnitState.BUILD && distTo(closestCompletableRuin) <= 18) || state == UnitState.BUILDSRP);// && rc.getPaint()>10;
    ...
    MapLocation rushdownTarget = null;
    RobotInfo enemyTargetTower = null;
    int numSoldiers = 0;
    private UnitState determineState() throws GameActionException{
        enemyTargetTower = getEnemyTargetTower();

        // OVERRIDES ============================================
        if(enemySpawnLocs != null && rc.getRoundNum() < 15){
            for(int i = enemySpawnLocs.length; --i >= 0; ) {
                if(enemySpawnLocs[i] == null) continue;
                if(rc.canSenseLocation(enemySpawnLocs[i])){
                    enemySpawnLocs[i] = null;
                    continue;
                }
                if(spawnLoc.distanceSquaredTo(enemySpawnLocs[i]) <= RUSHDOWN_DISTANCE_SQUARED){
                    rushdownTarget = enemySpawnLocs[i];
                    connected = true;
                    return UnitState.RUSHDOWN;
                }
            }
        }
        ...
        if(state == UnitState.COMBAT && enemyTargetTower != null && enemyTargetTower.health <= 50){
        ...
        numSoldiers = 0;
        if(enemyTargetTower != null && reachableFrom(rc.getLocation(), enemyTargetTower.location)){
            for (RobotInfo ally : allies) {
                if (ally.type == UnitType.SOLDIER && ally.location.distanceSquaredTo(enemyTargetTower.location) <= GameConstants.VISION_RADIUS_SQUARED) {
                    numSoldiers++;
                }
            }
            if (rc.getHealth() > ATTACK_HEALTH_THRESHOLD || enemyTargetTower.health <= 50){// && (numSoldiers > 0 || closestEnemyTower.health < 400)) {
                return UnitState.COMBAT;
            }
        }
        ...
        if (completableSRP != null && Math.min(rc.getNumberTowers(), mapData.friendlyTowers.size()) >= SRP_TOWER_THRESHOLD && mapData.friendlyTowers.paintTowers > 0 && enemies.length == 0){
...
//        }
    ...
    private void rushDownState() throws GameActionException{
        bugNav(rushdownTarget);
    }
    ...
    private void combatState() throws GameActionException {
        ...
        if(distTo(enemyTargetTower.getLocation()) <= rc.getType().actionRadiusSquared){
//            if(mapData.getMapInfo(rc.getLocation()).getPaint() == PaintType.EMPTY){
//                tryAttack(rc.getLocation());
//            }else{
                tryAttack(enemyTargetTower.getLocation());
//            }
            if(!tryMoveOutOfRange(enemyTargetTower.getLocation(), enemyTargetTower.type.actionRadiusSquared)){
                fuzzyMove(dirTo(enemyTargetTower.getLocation()).opposite());
            }
        }else{
            if(distTo(enemyTargetTower.location)>18){
                fuzzyMove(enemyTargetTower.location);
            }
            // get in attack range
            if((rc.getRoundNum()%2==0 || numSoldiers == 0) && rc.isActionReady()) {
                if (!tryMoveIntoRange(enemyTargetTower.getLocation(), rc.getType().actionRadiusSquared)) {
                    fuzzyMove(enemyTargetTower.getLocation());
                }
            }
//            if(mapData.getMapInfo(rc.getLocation()).getPaint() == PaintType.EMPTY){
//                tryAttack(rc.getLocation());
...
//                trySetIndicatorDot(loc, 0, 0, 0);
    ...
    private RobotInfo getEnemyTargetTower(){
        RobotInfo target = null;
        int minDist = 99999;
        boolean bestIsMoneyTower = false;
        for(RobotInfo enemy : enemies){
            if(enemy.type.isTowerType()){
                int dist = rc.getLocation().distanceSquaredTo(enemy.location);
//                boolean isMoneyTower = enemy.type.getBaseType() == UnitType.LEVEL_ONE_MONEY_TOWER;
                boolean isMoneyTower = false;
                if(isMoneyTower && !bestIsMoneyTower){
                    target = enemy;
                    bestIsMoneyTower = true;
                    minDist = dist;
                }else if(!bestIsMoneyTower && dist < minDist){
                    target = enemy;
                    minDist = dist;
                }else if(isMoneyTower == bestIsMoneyTower && dist < minDist){
                    target = enemy;
                    minDist = dist;
                }
            }
