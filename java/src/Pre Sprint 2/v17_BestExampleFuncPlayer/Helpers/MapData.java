package v17_BestExampleFuncPlayer.Helpers;

import battlecode.common.*;
import v17_BestExampleFuncPlayer.Helpers.Fast.FastIterableLocSet;
import v17_BestExampleFuncPlayer.Robot;
...
//    StringBuilder SRPExclusionZone2;
    ...
    public MapData(RobotController rc, int width, int height, Robot robot) {
        ...
        adjacentAllies = new int[width][height];
...
//        for(int i = height; --i >= 0;)  SRPExclusionZone[0][i] = SRPExclusionZone[1][i] = SRPExclusionZone[width - 1][i] = SRPExclusionZone[width - 2][i] = true;
    ...
    public void updateAdjacentAllies(RobotInfo[] allies){
        ...
        for(RobotInfo ally : allies){
            ...
            if(loc.x > 0) adjacentAllies[loc.x - 1][loc.y]++;
            if(loc.x < width - 1) adjacentAllies[loc.x + 1][loc.y]++;
            if(loc.y > 0) adjacentAllies[loc.x][loc.y - 1]++;
            if(loc.y < height - 1) adjacentAllies[loc.x][loc.y + 1]++;
            if(loc.x > 0 && loc.y > 0) adjacentAllies[loc.x - 1][loc.y - 1]++;
            if(loc.x > 0 && loc.y < height - 1) adjacentAllies[loc.x - 1][loc.y + 1]++;
            if(loc.x < width - 1 && loc.y > 0) adjacentAllies[loc.x + 1][loc.y - 1]++;
            if(loc.x < width - 1 && loc.y < height - 1) adjacentAllies[loc.x + 1][loc.y + 1]++;
        }
    }
    ...
    public void updateLandmarks(MapLocation[] ruins, RobotInfo[] allies, MapLocation currentLocation) throws GameActionException{
        // add new stuff
...
//            }
            ...
            if(info.getMark() == SRP_COLOR){
                ...
                if(ruinDir != null && !rc.canSenseRobotAtLocation(infoLoc.add(ruinDir))){
                    markPattern(infoLoc.add(ruinDir), robot.markDirectionToTowerType(ruinDir.opposite()));
                }
            }
        }
